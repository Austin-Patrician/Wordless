import type { AssistantMessage, ImageContent, Model, Models, UserMessage } from "@wordless/ai";
...
> {
	...
	/** Continue from the active session context without adding another user message. */
	async continue(): Promise<AssistantMessage> {
		...
		try {
			...
			if (!lastMessage) throw new AgentHarnessError("invalid_state", "Cannot continue without session context");
			if (lastMessage.role === "assistant") {
				throw new AgentHarnessError("invalid_state", "Cannot continue from an assistant message");
			}
	...
	async prepareContextOverflowRecovery(): Promise<{ failedMessageEntryId: string }> {
		...
		try {
			...
			return { failedMessageEntryId: entry.id };
		} catch (error) {
	...
	async compact(
		...
		try {
			...
			const instructions = hookResult?.customInstructions ?? customInstructions;
			const compactResult = provided
				? { ok: true as const, value: provided }
				: await compact(preparation, this.models, model, instructions, abortController.signal, this.thinkingLevel);
