...
Stateful agent with tool execution and event streaming. Built on `@wordless/ai`.

## Installation

```bash
npm install @wordless/agent
```

## Quick Start

```typescript
import { Agent } from "@wordless/agent";
import { getModel } from "@wordless/ai";
...
```typescript
import { Agent, streamProxy } from "@wordless/agent";
...
```typescript
import { agentLoop, agentLoopContinue } from "@wordless/agent";
