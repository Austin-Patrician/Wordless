...
- Removed the `@wordless/agent/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.
...
- Added `@wordless/agent/base` for bundlers that want to pair the agent core with selective `@wordless/ai/base` provider registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
