...
> Internal Wordless fork. See `../../UPSTREAM.md` and `WORDLESS_CHANGES.md` before modifying or updating this package.
...
```bash
npm install @wordless/ai
```

TypeBox exports are re-exported from `@wordless/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@wordless/ai';
import { builtinModels } from '@wordless/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@wordless/ai/providers/anthropic';
import { openaiProvider } from '@wordless/ai/providers/openai';
import { openrouterProvider } from '@wordless/ai/providers/openrouter';
import { amazonBedrockProvider } from '@wordless/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@wordless/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@wordless/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@wordless/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@wordless/ai';
...
```typescript
import { validateToolCall, type Tool } from '@wordless/ai';
...
```typescript
import { builtinImagesModels } from '@wordless/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@wordless/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@wordless/ai';
...
```typescript
import { stream } from '@wordless/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@wordless/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@wordless/ai';
import { anthropicProvider } from '@wordless/ai/providers/anthropic';
import { openaiProvider } from '@wordless/ai/providers/openai';
import { googleProvider } from '@wordless/ai/providers/google';
...
```typescript
import { createModels } from '@wordless/ai';
import { anthropicProvider } from '@wordless/ai/providers/anthropic';
...
- `@wordless/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@wordless/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@wordless/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@wordless/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@wordless/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@wordless/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@wordless/ai/bedrock-provider';
...
```bash
npx @wordless/ai login              # interactive provider selection
npx @wordless/ai login anthropic    # login to specific provider
npx @wordless/ai list               # list available providers
```
...
The legacy flow functions remain available via the `@wordless/ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@wordless/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@wordless/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@wordless/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@wordless/ai`.
