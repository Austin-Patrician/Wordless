...
export function resolveProviderAuth(
	...
	return raceWithAbortSignal(
		resolveProviderAuthWithSignal(provider, credentials, authContext, overrides, signal, model),
				signal,
	);
}
...
async function resolveProviderAuthWithSignal(
	...
	if (stored) {
		...
		if (stored.type === "api_key" && provider.auth.apiKey) {
			...
			return resolveApiKey(requestAuthContext, provider.auth.apiKey, provider.id, credential, signal, model);
		}
		return undefined;
	}
	...
	return provider.auth.apiKey
		? resolveApiKey(requestAuthContext, provider.auth.apiKey, provider.id, undefined, signal, model)
		: undefined;
}
...
async function resolveApiKey(
	...
	try {
		return await apiKey.resolve({ model, ctx: authContext, credential, signal });
	} catch (error) {
