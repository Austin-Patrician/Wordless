...
class ModelsImpl implements MutableModels {
	...
	async getAuth(
		...
		const provider = this.providers.get(providerId);
		if (!provider) return undefined;
		const result = await resolveProviderAuth(provider, this.credentials, this.authContext, { ...overrides, signal }, typeof providerOrModel === "string" ? undefined : providerOrModel);
