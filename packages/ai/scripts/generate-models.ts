...
function readGeneratorOptions(args: string[]): {
	...
	for (let index = 0; index < args.length; index++) {
		...
		if (arg === "--proxy") {
			const value = args[++index];
			if (!value) throw new Error("--proxy requires a URL");
			let parsed: URL;
			try {
			...
			if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
				throw new Error(`Proxy must use http or https: ${value}`);
			}
	...
	return { strict, dataOnly, jsonOnly, jsonOutputDir, pretty, proxy };
}

const generatorOptions = readGeneratorOptions(process.argv.slice(2));

function configureHttpProxy(proxy: string | undefined): void {
	if (proxy) {
...
async function generateModels() {
	configureHttpProxy(generatorOptions.proxy);
