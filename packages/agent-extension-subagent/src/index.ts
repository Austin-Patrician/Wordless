...
const DEFAULT_ROLES: SubagentRoleDefinition[] = [
  {
    id: "scout",
    name: "Scout",
    description: "Discover information. Search, inspect and summarize.",
    model: null,
  },
  {
    id: "planner",
    name: "Planner",
    description:
      "Produce execution plans and break complex goals into actionable steps.",
    model: null,
  },
  {
    id: "reviewer",
    name: "Reviewer",
    description:
      "Inspect completed work, verify correctness, and identify risks.",
    model: null,
  },
  {
    id: "worker",
    name: "Worker",
    description: "Execute a concrete implementation task.",
    model: null,
  },
  {
    id: "researcher",
    name: "Researcher",
    description: "Collect source-grounded evidence for one research dimension.",
    model: null,
  },
  {
    ...
    description:
      "Review source-grounded evidence and identify gaps or conflicts.",
    model: null,
  },
];
...
const DelegationTaskSchema = Type.Object({
  agent: Type.Union([
    ...
    Type.Literal("research-reviewer"),
  ]),
...
export const subagentExtension: AgentExtensionDefinition = {
  descriptor: {
    ...
    name: "Subagent",
    description:
      "Delegate isolated research, planning, review, and implementation tasks.",
    ...
    return {
      async activate() {
        if (!context.subagentRunner)
          throw new Error("Subagent execution is unavailable for this session");
        await context.registerTools([
          createDelegateTool(context, roles, () => reviewerAllowed),
        ]);
        context.harness.on("before_agent_start", (event) => {
          reviewerAllowed =
            hasExplicitReviewIntent(context.getCurrentPrompt() ?? "");
          return {
...
function createDelegateTool(
  ...
  return {
    ...
    label: "Delegate task",
    description:
      "Delegate work with exactly { mode, tasks }. Each task is exactly { agent, task }. mode=single requires one task; mode=parallel runs independent read-only tasks; mode=sequential passes each result to the next task. Example: { mode: 'single', tasks: [{ agent: 'reviewer', task: 'Review the current Git workspace changes' }] }.",
    ...
    async execute(_toolCallId, params, signal, onUpdate) {
      ...
      for (const task of params.tasks) {
        if (!availableRoles.has(task.agent))
          throw new Error(`The ${task.agent} role is unavailable`);
      }
      ...
      const update = (progress: SubagentTaskProgress) => {
        ...
        if (progress.tool !== undefined) {
          const eventId = progress.tool.callId
          ...
          const index = current.events.findIndex((item) => item.id === eventId);
          if (index === -1) current.events.push(event);
          else current.events[index] = event;
        }
        if (progress.output !== undefined && progress.output.trim()) {
          const event = {
            id: `output:${current.id}`,
        ...
        onUpdate?.({
          content: [{ type: "text", text: formatDelegationSummary(details) }],
...
function validateDelegation(
  mode: DelegationParams["mode"],
  tasks: DelegationTask[],
  reviewerAllowed: boolean,
): void {
  if (mode === "single" && tasks.length !== 1)
    throw new Error("single mode requires exactly one task");
  if ((mode === "parallel" || mode === "sequential") && tasks.length < 2)
    throw new Error(`${mode} mode requires at least two tasks`);
  for (const task of tasks) {
    if (task.agent === "reviewer" && !reviewerAllowed)
      throw new Error(
        "Reviewer delegation is only available when the user explicitly asks for review",
      );
  }
  if (mode === "parallel" && tasks.some((task) => task.agent === "worker"))
    throw new Error(
      "Worker tasks cannot run in parallel until their file ownership is explicitly coordinated",
    );
}
...
function expectedOutput(agent: DelegationTask["agent"]): string {
  return agent === "scout"
    ? "A concise findings summary with file references"
    : agent === "researcher"
      ? "Structured source-grounded claims submitted for one research dimension"
      : agent === "research-reviewer"
        ? "A review of evidence support, gaps, conflicts, and suggested supplement research"
        : agent === "planner"
          ? "An actionable implementation plan"
          : agent === "worker"
            ? "A concise implementation and verification summary"
            : "A review of correctness, risks, and suggested improvements";
}

function configuredRoleModels(
  settings: JsonObject,
): Partial<
  Record<
    SubagentRoleDefinition["id"],
...
function hasExplicitReviewIntent(prompt: string): boolean {
  ...
  return /(code\s+review|review(?:er)?\b|audit\b|second\s+opinion|subagent\s+reviewer|审查|审核|代码评审|(?:检查|查看).{0,18}(当前|工作区|git|变更|修改|diff|问题))/.test(
    normalized,
  );
}
