      ...
      <div className="mx-auto max-w-[700px]">
        ...
        <section className="mb-7">
          ...
          {customConfiguration ? <><textarea
            ...
            value={raw}
          /><p className="mt-2 text-[12px] text-muted-foreground">baseUrl, headers, compat, models and modelOverrides are written to models.json.</p></> : null}
        </section>
        <section>
...
function configurationExample(kind: ConfiguredModelKind, customProvider: boolean): string {
  ...
  if (customProvider) {
    return JSON.stringify({ name: "My provider", baseUrl: "https://api.example.com/v1", api: "openai-completions", headers: { "X-Client": "Wordless" }, models: [{ id: "model-id", name: "Model name", input: ["text"], contextWindow: 128000, maxTokens: 16384 }] }, null, 2);
  }
  return JSON.stringify({ baseUrl: "https://api.example.com/v1", headers: { "X-Client": "Wordless" }, modelOverrides: { "model-id": { reasoning: true, contextWindow: 128000 } } }, null, 2);
}
