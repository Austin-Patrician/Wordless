...
import { backgroundIsAnimated, backgroundPreviewStyle, backgroundRenderSource, builtinBackgrounds } from "../appearance/backgrounds";
...
export function PersonalizationSettings() {
  ...
  const { appearance, previewAppearance, reduceMotion, setAppearance, t } = usePreferences();
  ...
  const uploadBackground = async (file: File) => {
    ...
    try {
      ...
      const animated = asset.animated === true;
      await setAppearance({ ...appearance, background: { ...background, fit: animated ? "cover" : background.fit, source: { kind: "custom", assetId: asset.assetId, ...(animated ? { animated: true, posterAssetId: asset.posterAssetId } : {}) } } });
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-5 sm:p-8">
      <div className="mx-auto max-w-[680px] space-y-4">
        <section className="overflow-hidden rounded-[10px] border border-[#e2e2dc] bg-white dark:border-border dark:bg-card">
          ...
          <div className="relative aspect-[16/7] overflow-hidden bg-[#f1f1ed] dark:bg-[#24261f]"><div className="absolute inset-0" style={{ ...backgroundPreviewStyle(backgroundRenderSource(background.source, reduceMotion), backgroundIsAnimated(background.source) ? "cover" : background.fit, background.position), filter: background.blurPx > 0 ? `blur(${background.blurPx}px)` : undefined, opacity: background.intensity / 100, transform: background.blurPx > 0 ? "scale(1.025)" : undefined }} /><div className="absolute inset-0" style={{ background: "var(--wordless-background-scrim)" }} /><div className="absolute inset-x-5 top-4 h-5 rounded-[4px] border border-[#d7d7d1]/80 bg-white/80 dark:border-white/10 dark:bg-[#262820]/80" /><div className="absolute bottom-4 left-5 w-[42%] rounded-[6px] border border-[#deded8]/80 bg-white/88 p-2.5 shadow-[0_4px_12px_rgba(0,0,0,0.06)] dark:border-white/10 dark:bg-[#202219]/88"><span className="block h-1.5 w-24 rounded-full bg-[#44443f]/75 dark:bg-[#e9ece0]/75" /><span className="mt-2 block h-1.5 w-full rounded-full bg-[#a3a39c]/45" /><span className="mt-1.5 block h-1.5 w-3/4 rounded-full bg-[#a3a39c]/35" /></div></div>
        </section>
        ...
        <section className="rounded-[10px] bg-[#f7f7f5] p-4 dark:bg-[#22241c]">
          ...
          <input accept="image/png,image/jpeg,image/webp,image/gif,.gif" className="sr-only" onChange={(event) => { const file = event.target.files?.[0]; event.currentTarget.value = ""; if (file) void uploadBackground(file); }} ref={fileInputRef} type="file" />
