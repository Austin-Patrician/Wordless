...
export function MediaCanvas({ fullscreen, leftOpen, onBackToLibrary, onOpenModels, onToggleFullscreen, onToggleLeft, sessionId }: MediaCanvasProps) {
  ...
  const downloadMediaAsset = useCallback(async (assetId: string) => {
    ...
    try {
      ...
      setDownloadNotice(locale === "zh-CN" ? "图片已下载到下载文件夹" : "Image downloaded to Downloads");
    } catch (cause) { setError(errorMessage(cause)); }
...
function DownloadNotice({ locale, onDismiss }: { locale: Locale; onDismiss: () => void }) {
  ...
  return <div aria-live="polite" className="absolute right-4 top-4 z-50 flex w-[min(292px,calc(100%-2rem))] items-start gap-2.5 rounded-[8px] border border-[#a7bd68]/55 bg-card/95 px-3 py-2.5 text-foreground shadow-[0_12px_28px_rgba(32,38,22,.18)] backdrop-blur-sm">
    ...
    <div className="min-w-0 flex-1 pt-px"><p className="text-[11px] font-semibold leading-4">{chinese ? "下载完成" : "Download complete"}</p><p className="mt-0.5 text-[10px] leading-4 text-muted-foreground">{chinese ? "图片已保存至下载文件夹" : "Image saved to your Downloads folder"}</p></div>
    <button aria-label={chinese ? "关闭下载提示" : "Dismiss download notification"} className="-mr-1 -mt-1 grid h-6 w-6 shrink-0 place-items-center rounded-[4px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground" onClick={onDismiss} type="button"><X className="h-3.5 w-3.5" /></button>
  </div>;
}
...
function NodeActionModelPicker({ definition, locale, models, onCancel, onSelect, preferredModelKey }: { definition: MediaOperationDefinition; locale: Locale; models: ConfiguredModelSummary[]; onCancel: () => void; onSelect: (modelKey: string) => void; preferredModelKey: string }) {
  ...
  return <section className="w-[310px] rounded-[8px] border border-[#575b51] bg-card p-2.5 text-card-foreground shadow-[0_16px_42px_rgba(0,0,0,.18)] dark:border-[#54584e]" onClick={(event) => event.stopPropagation()}><div className="mb-2 flex items-center justify-between gap-3"><span className="flex min-w-0 items-center gap-2 text-[12px] font-medium"><Icon className="h-3.5 w-3.5 text-muted-foreground" /><span className="truncate">{definition.label[locale]}</span></span><button aria-label={locale === "zh-CN" ? "取消" : "Cancel"} className="grid h-6 w-6 place-items-center rounded-[4px] text-muted-foreground hover:bg-muted hover:text-foreground" onClick={onCancel} type="button"><X className="h-3.5 w-3.5" /></button></div>{models.length ? <div className="flex gap-1.5"><Select onValueChange={setModelKey} value={modelKey || undefined}><SelectTrigger aria-label={locale === "zh-CN" ? "选择兼容模型" : "Select a compatible model"} className="h-8 min-w-0 flex-1 border-input bg-background px-2 text-[11px] shadow-none"><SelectValue placeholder={locale === "zh-CN" ? "选择兼容模型" : "Select a compatible model"} /></SelectTrigger><SelectContent className="max-h-64">{models.map((model) => <SelectItem key={`${model.providerId}:${model.modelId}`} value={`${model.providerId}:${model.modelId}`}><span className="flex min-w-0 items-center gap-2"><ProviderIcon avatarId={model.providerAvatarId} className="h-3.5 w-3.5 shrink-0 object-contain" providerId={model.providerId} /><span className="min-w-0 truncate">{model.displayName}</span></span></SelectItem>)}</SelectContent></Select><button aria-label={definition.label[locale]} className="grid h-8 w-8 shrink-0 place-items-center rounded-[5px] bg-accent text-accent-foreground disabled:cursor-not-allowed disabled:opacity-40" disabled={!modelKey} onClick={() => onSelect(modelKey)} title={definition.label[locale]} type="button"><Send className="h-3.5 w-3.5" /></button></div> : <p className="rounded-[5px] bg-muted px-2.5 py-2 text-[10px] leading-4 text-muted-foreground">{locale === "zh-CN" ? "没有已启用的兼容图片模型。" : "No enabled image model supports this action."}</p>}</section>;
}
