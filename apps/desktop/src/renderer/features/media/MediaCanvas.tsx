...
type MediaNodeContextMenu =
  | { kind: "asset"; asset: MediaAsset; x: number; y: number }
  | { kind: "reference-composer"; composerId: string; x: number; y: number };
...
export function MediaCanvas({ fullscreen, leftOpen, onBackToLibrary, onOpenModels, onToggleFullscreen, onToggleLeft, sessionId }: MediaCanvasProps) {
  ...
  const removeReferenceComposer = useCallback((composerId: string) => {
    setMediaContextMenu(null);
    setSelectedReferenceComposerId((current) => current === composerId ? null : current);
    setReferenceComposers((current) => current.filter((draft) => draft.id !== composerId));
  }, []);
  ...
  return <section className="relative flex min-w-0 flex-1 overflow-hidden bg-background">
    ...
    <div className="relative min-w-0 flex-1 pt-11" onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "copy"; }} onDrop={(event) => { event.preventDefault(); const position = flow?.screenToFlowPosition({ x: event.clientX, y: event.clientY }); void importFiles([...event.dataTransfer.files], position); }} onMouseMove={(event) => { const position = flow?.screenToFlowPosition({ x: event.clientX, y: event.clientY }); if (position) lastCanvasPositionRef.current = position; }}>
      ...
      {mediaContextMenu ? <div className="fixed z-[100] w-[136px] rounded-[7px] border border-border bg-card py-1 text-[11px] text-foreground shadow-[0_8px_18px_rgba(0,0,0,.16)]" onPointerDown={(event) => event.stopPropagation()} style={{ left: mediaContextMenu.x, top: mediaContextMenu.y }}>{mediaContextMenu.kind === "asset" ? <><button className="flex h-7 w-full items-center gap-2 px-2.5 text-left hover:bg-muted disabled:cursor-not-allowed disabled:opacity-40" disabled={mediaContextMenu.asset.status !== "ready"} onClick={() => copyMediaNode(mediaContextMenu.asset.id)} type="button"><Copy className="h-3.5 w-3.5" />{t("mediaCopyNode")}</button><button className="flex h-7 w-full items-center gap-2 px-2.5 text-left text-red-600 hover:bg-red-500/10 disabled:cursor-not-allowed disabled:opacity-40 dark:text-red-300" disabled={mediaContextMenu.asset.status === "rendering"} onClick={() => void deleteMediaNode(mediaContextMenu.asset.id)} type="button"><Trash2 className="h-3.5 w-3.5" />{t("mediaDeleteNode")}</button></> : <button className="flex h-7 w-full items-center gap-2 px-2.5 text-left text-red-600 hover:bg-red-500/10 dark:text-red-300" onClick={() => removeReferenceComposer(mediaContextMenu.composerId)} type="button"><Trash2 className="h-3.5 w-3.5" />{t("mediaDeleteNode")}</button>}</div> : null}
