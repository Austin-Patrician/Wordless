...
import { Archive, ArrowDown, ChevronDown, ChevronUp, CircleAlert, Copy, FileText, Folder, Layers3, LoaderCircle, RotateCcw } from "lucide-react";
      ...
      <div className={`group relative min-h-px ${isFirstMessage ? "pt-4" : "pt-8"} outline-none focus-visible:ring-2 focus-visible:ring-ring ${contentBlocks.length > 0 ? "pb-7" : ""}`} data-thread-message-id={message.id} tabIndex={-1}>
        <div className="ml-auto flex w-fit max-w-[88%] flex-col items-end sm:max-w-[560px]">
          {contentBlocks.length > 0 ? <div className="w-fit max-w-full break-words rounded-[10px] bg-[#f0f0ed] px-3.5 py-2.5 text-[14px] leading-6 text-[#343431] dark:bg-muted dark:text-foreground"><CollapsibleUserMessage contentKey={message.id}>{contentBlocks.map((block, index) => block.type === "text"
            ...
            : block.type === "workspace-reference" ? <span className="mx-1.5 inline-flex h-6 max-w-[230px] select-none items-center gap-1 rounded-[5px] border border-[#bed7cf] bg-[#eef8f5] px-1.5 align-bottom text-[13px] font-normal leading-4 text-[#34574d] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] dark:border-[#3b675c] dark:bg-[#20332d] dark:text-[#c5e3d9]" key={block.id} title={block.path}>{block.kind === "directory" ? <Folder aria-hidden className="h-3 w-3 shrink-0 text-[#4f8b79]" /> : <FileText aria-hidden className="h-3 w-3 shrink-0 text-[#4f8b79]" />}<span className="min-w-0 truncate">{block.name}</span></span>
            : block.type === "artifact" ? <span className="mx-1.5 inline-flex h-6 max-w-[250px] select-none items-center gap-1 rounded-[5px] border border-[#b8d6cb] bg-[#edf8f4] px-1.5 align-bottom text-[13px] font-normal leading-4 text-[#345f53] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] dark:border-[#416b5e] dark:bg-[#20372f] dark:text-[#bae2d3]" key={`${block.artifactId}:${block.surfaceId ?? block.name}`} title={block.locator ?? block.name}><Layers3 aria-hidden className="h-3 w-3 shrink-0 text-[#4f8b79] dark:text-[#9ccfbd]" /><span className="min-w-0 truncate">{block.name}</span></span>
            : <span className="mx-1.5 inline-flex h-6 max-w-[210px] select-none items-center gap-1 rounded-[5px] border border-[#deded9] bg-[#f8f8f6] px-1.5 align-bottom text-[13px] font-normal leading-4 text-[#45453f] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] dark:border-[#4b4c45] dark:bg-[#2b2c27] dark:text-[#deded8]" key={block.id} title={block.name}><span aria-hidden className="grid h-4 w-4 shrink-0 place-items-center rounded-[4px] bg-[#e6e7e1] text-[9px] font-semibold leading-none text-[#5b5c55] dark:bg-[#41423b] dark:text-[#d0d1c9]">{skillIconText(block.name)}</span><span className="min-w-0 truncate">{block.name}</span></span>)}</CollapsibleUserMessage></div> : null}
        ...
        <Virtuoso
          ...
          itemContent={(index, item) => {
            ...
            return <ThreadContentFrame className={followsCompaction ? "pt-[26px]" : ""} densityRail={showDensityRail}>{item.type === "compaction"
              ...
              : <MessageBody canPlan={canPlan} isFirstMessage={item.messages[0]?.id === snapshot.messages[0]?.id} messages={item.messages} onEnableAutoApprove={snapshot.toolApprovalMode === "manual" ? () => setToolApprovalMode("auto") : undefined} onHandoffClarification={handoffClarification} onLoadToolOutput={loadToolOutput} onOpenResearchTask={onOpenResearchTask} onResolveApproval={resolveApproval} onResolveClarificationQuestion={resolveClarificationQuestion} onResolvePlanResult={resolvePlanResult} onResolveUserRequest={resolveUserRequest} planMode={planMode} runPresentation={runPresentation?.userMessageId && item.turnId === `turn:${runPresentation.userMessageId}` ? runPresentation : null} showFooter={!snapshot.isRunning && index === firstItemIndex + timeline.length - 1} workbenchId={snapshot.session.workbenchId} />}</ThreadContentFrame>;
          }}
          rangeChanged={(range) => {
