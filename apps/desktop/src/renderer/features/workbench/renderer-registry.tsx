...
function ToolOutput({ block, onLoadToolOutput }: Pick<ToolActivityProps, "block" | "onLoadToolOutput">) {
  ...
  return <details className="group mt-2" onToggle={(event) => { if ((event.currentTarget as HTMLDetailsElement).open) load(); }}><summary className="flex w-fit cursor-pointer list-none items-center gap-1 font-mono text-[11px] text-[#777770] outline-none focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden">{loading ? "Loading output" : "View output"}<ChevronDown aria-hidden className="h-3.5 w-3.5 text-[#999991] transition-transform duration-150 group-open:rotate-180" /></summary><pre className="m-0 mt-2 max-h-52 overflow-auto bg-[#fafaf9] px-3 py-2 font-mono text-[11px] leading-5 text-[#4d4d47] dark:bg-muted dark:text-muted-foreground">{block.output}</pre></details>;
}
...
function nestedApproval(task: SubagentTaskDetails): MessageToolBlock["approval"] | undefined {
  ...
  if (!value || typeof value.approvalId !== "string" || (value.status !== "required" && value.status !== "approved" && value.status !== "rejected") || (value.risk !== "file-write" && value.risk !== "command" && value.risk !== "connector" && value.risk !== "workspace-access") || (value.severity !== "normal" && value.severity !== "high") || typeof value.summary !== "string" || !preview || typeof preview.type !== "string" || !Array.isArray(value.matchedRules)) return undefined;
    ...
    <div className="mt-2 divide-y divide-[#e7e7e2] border-y border-[#e7e7e2] dark:divide-border dark:border-border">
      {details.tasks.map((task) => {
        ...
        return <div className="px-1 py-3" key={task.id}>
          ...
          {task.output ? <details className="group mt-2"><summary className="flex w-fit cursor-pointer list-none items-center gap-1 font-mono text-[10px] text-[#777770] outline-none focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden">View result<ChevronDown aria-hidden className="h-3.5 w-3.5 text-[#999991] transition-transform duration-150 group-open:rotate-180" /></summary><pre className="m-0 mt-2 max-h-60 overflow-auto bg-[#fafaf9] px-3 py-2 whitespace-pre-wrap font-mono text-[11px] leading-5 text-[#4d4d47] dark:bg-muted dark:text-muted-foreground">{task.output}</pre></details> : null}
...
function CodeToolActivity({ block, onEnableAutoApprove, onLoadToolOutput, onResolveApproval }: ToolActivityProps) {
  ...
  return (
    <section className="py-3">
      ...
      {oldText !== undefined && newText !== undefined ? <details className="group mt-2"><summary className="flex w-fit cursor-pointer list-none items-center gap-1 font-mono text-[11px] font-semibold text-[#777770] outline-none focus-visible:ring-2 focus-visible:ring-ring [&::-webkit-details-marker]:hidden">View change<ChevronDown aria-hidden className="h-3.5 w-3.5 text-[#999991] transition-transform duration-150 group-open:rotate-180" /></summary><div className="mt-2 max-h-80 overflow-auto border-y border-[#e1e1dc] font-mono text-[11px] leading-5 dark:border-border"><pre className="m-0 min-w-max whitespace-pre bg-[#fbefec] px-3 text-[#9a564b] dark:bg-[#3a211d] dark:text-[#ffb4a8]">- {oldText}</pre><pre className="m-0 min-w-max whitespace-pre bg-[#eff7e7] px-3 text-[#547c36] dark:bg-[#29351d] dark:text-[#d8f28a]">+ {newText}</pre></div></details> : null}
...
function ToolApprovalCard({ block, onEnableAutoApprove, onResolveApproval }: ToolActivityProps) {
  ...
  const elevated = approval.requiresElevation === true || approval.risk === "workspace-access";
