import { Button, Dialog, DialogClose, DialogContent, DialogTitle, Switch, cn } from "@wordless/ui-kit";
import { Cable, CheckCircle2, Cloud, KeyRound, LoaderCircle, Plus, Terminal, Trash2, X } from "lucide-react";
...
const templateIcons: Record<Exclude<ConnectorTemplateId, null>, string> = {
  feishu: feishuIcon,
...
function ConnectorIcon({ templateId, transport }: Pick<ConnectorSummary, "templateId" | "transport">) {
  const icon = templateId === null ? undefined : templateIcons[templateId];
  if (icon) return <img alt="" className="h-5 w-5 object-contain" src={icon} />;
  return transport === "stdio" ? <Terminal className="h-4 w-4" /> : <Cloud className="h-4 w-4" />;
}
...
export function ConnectorsView({ dialogOpen, onDialogOpenChange, query }: ConnectorsViewProps) {
  ...
  const save = async () => {
    ...
    setError(null);
    try {
      const connector = await client.saveConnector(draft);
      if (draft.transport === "streamable-http" && draft.templateId !== null) await client.authorizeConnector(connector.id);
  ...
  return <section className="min-h-0 flex-1 overflow-y-auto pt-8">
    ...
    <Dialog onOpenChange={(open) => { onDialogOpenChange(open); if (!open) setDraft(newDraft()); }} open={dialogOpen}>
      <DialogContent className="w-[min(42rem,calc(100vw-2rem))] rounded-[12px] px-6 py-6" showCloseButton={false}>
        ...
        <div className="mt-6 flex items-center justify-between border-t border-[#ecece7] pt-4 dark:border-border"><span className="text-[10px] text-muted-foreground">官方模板会在保存后打开浏览器进行 OAuth 授权。</span><Button disabled={busy === "save"} onClick={() => void save()} type="button">{busy === "save" ? <LoaderCircle className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />}{draft.templateId && draft.transport === "streamable-http" ? "添加并连接" : "保存并测试"}</Button></div>
      </DialogContent>
    </Dialog>
  </section>;
}
