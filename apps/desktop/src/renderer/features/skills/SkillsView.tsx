...
import { ArrowLeft, Cable, CircleAlert, Code2, Command, FileText, Palette, RefreshCw, Search, Trash2, Upload } from "lucide-react";
...
function skillIcon(skill: SkillSummary) {
  const subject = `${skill.name} ${skill.description}`.toLowerCase();
  if (/design|ui|review|figma/.test(subject)) return Palette;
  if (/code|repo|develop|test|debug/.test(subject)) return Code2;
  if (/write|document|release|note/.test(subject)) return FileText;
  return Command;
}
