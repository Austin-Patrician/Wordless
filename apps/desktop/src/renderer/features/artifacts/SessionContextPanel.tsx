...
export function SessionContextPanel({ collapsed, fullscreen, leftSidebarWidth, minimumMainWidth, onFullscreen, onToggle, onViewChange, renderContent, contentClassName, showFooter = true, showMenu = true, tabs, view }: SessionContextPanelProps) {
  ...
  return <aside className={`relative flex min-h-0 min-w-0 max-w-full shrink-0 flex-col overflow-hidden bg-[var(--wordless-shell-context)] ${fullscreen ? "h-full flex-1" : "hidden border-l border-[#e4e4df] lg:flex dark:border-border"}`} style={fullscreen ? undefined : { width: renderedWidth }}>
