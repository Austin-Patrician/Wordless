...
export function SessionContextPanel({ collapsed, fullscreen, leftSidebarWidth, minimumMainWidth, onFullscreen, onToggle, onViewChange, renderContent, contentClassName, showFooter = true, showMenu = true, tabs, view }: SessionContextPanelProps) {
  ...
  return <aside className={`relative flex min-h-0 shrink-0 flex-col bg-[var(--wordless-shell-context)] ${fullscreen ? "h-full flex-1" : "hidden border-l border-[#e4e4df] lg:flex dark:border-border"}`} style={fullscreen ? undefined : { width: renderedWidth }}>
    ...
    <header className="flex h-[52px] shrink-0 items-center justify-between border-b border-[#e4e4df] px-3 dark:border-border">{showMenu ? <button aria-label="Context menu" className="grid h-7 w-7 place-items-center rounded-[6px] text-[#65655f] hover:bg-[#f0f0ec] dark:text-muted-foreground dark:hover:bg-muted" type="button"><Menu className="h-4 w-4" /></button> : <span className="h-7 w-7" /> }<div className="flex items-center gap-0.5"><button aria-label={fullscreen ? "退出全屏" : "全屏"} className="grid h-7 w-7 place-items-center rounded-[6px] text-[#65655f] hover:bg-[#f0f0ec] dark:text-muted-foreground dark:hover:bg-muted" onClick={onFullscreen} type="button">{fullscreen ? <Minimize2 className="h-3.5 w-3.5" /> : <Maximize2 className="h-3.5 w-3.5" />}</button><button aria-label="收起右栏" className="grid h-7 w-7 place-items-center rounded-[6px] text-[#65655f] hover:bg-[#f0f0ec] dark:text-muted-foreground dark:hover:bg-muted" onClick={onToggle} type="button"><PanelRightClose className="h-3.5 w-3.5" /></button></div></header>
    ...
    <div className={`flex min-h-0 flex-1 flex-col ${contentClassName ?? "overflow-y-auto"}`}>{renderContent(view)}</div>
    {showFooter ? <footer className="shrink-0 border-t border-[#e4e4df] px-4 py-2 font-mono text-[10px] text-[#a8a8a2] dark:border-border">{tabs.find((item) => item.id === view)?.label ?? "Context"}</footer> : null}
  </aside>;
}
