...
import type { ArtifactDescriptor, ArtifactPreviewManifest, OfficeEngineHealth, SpreadsheetRangeProfile, SpreadsheetSelection } from "@wordless/protocol";
import { BarChart3, ExternalLink, FolderOpen, FunctionSquare, LoaderCircle, RefreshCw, Sigma, Table2 } from "lucide-react";
...
export function SpreadsheetContextPanel({ onArtifactSelection, sessionId }: WorkbenchContextPanelProps) {
  ...
  const refreshArtifact = useCallback(async (artifactId: string) => {
    ...
    setError(null);
    try {
      const nextPreview = await client.getSpreadsheetPreview(sessionId, artifactId);
  ...
  }, [client, selectedArtifactId, sessionId]);
