...
export function ResearchInspector({ client, run, selection, sessionId, onBack }: Props) {
  ...
  const selectedDimension = run?.research?.dimensions.find((dimension) => dimension.id === selection.dimensionId);
  const claims = useMemo(() => run?.research?.claims.filter((claim) => claim.dimensionId === selection.dimensionId) ?? [], [run, selection.dimensionId]);
  const sourceIds = useMemo(() => new Set(claims.flatMap((claim) => claim.evidenceRefs)), [claims]);
  const sources = run?.research?.sources.filter((source) => sourceIds.has(source.id)) ?? [];

  return <section className="min-h-0 flex-1 overflow-y-auto px-4 py-3">
    <header className="border-b border-[#e5e5df] pb-3 dark:border-border">
      <button aria-label={chinese ? "返回研究总览" : "Back to research overview"} className="mb-3 flex items-center gap-1 text-[10px] font-medium text-[#687b4d] hover:text-[#465a31] dark:text-[#c3df85]" onClick={onBack} type="button"><ArrowLeft className="h-3.5 w-3.5" />{chinese ? "研究总览" : "Research overview"}</button>
      <div className="flex items-start gap-2"><div className="grid h-7 w-7 shrink-0 place-items-center rounded-[6px] bg-[#edf2df] text-[#607a35] dark:bg-[#29351d] dark:text-[#c8e883]"><BookOpenCheck className="h-3.5 w-3.5" /></div><div className="min-w-0 flex-1"><p className="truncate text-[11px] font-semibold">{selectedDimension?.name ?? task?.dimensionName ?? selection.dimensionId}</p><p className="mt-0.5 line-clamp-2 text-[9px] leading-4 text-muted-foreground">{selectedDimension?.question ?? task?.question}</p></div></div>
    ...
    {!task ? <div className="grid min-h-40 place-items-center text-center text-[10px] text-muted-foreground"><FileSearch className="h-5 w-5 opacity-50" /><p className="mt-2">{chinese ? "正在加载研究轨迹…" : "Loading research trace…"}</p></div> : tab === "activity" ? <InspectorActivity chinese={chinese} task={task} /> : tab === "evidence" ? <div className="mt-3 space-y-2">{sources.length === 0 ? <p className="py-8 text-center text-[10px] text-muted-foreground">{chinese ? "该维度还没有已确认来源。" : "No confirmed sources for this dimension yet."}</p> : sources.map((source) => <button className="flex w-full items-start gap-2 border-b border-border/70 py-2 text-left hover:text-[#587846]" key={source.id} onClick={() => void client.openExternalUrl(source.url)} type="button"><span className="min-w-0 flex-1"><span className="block text-[10px] font-medium leading-4">{source.title}</span><span className="mt-0.5 block truncate font-mono text-[8px] text-muted-foreground">{source.publisher ?? source.url}</span></span><ExternalLink className="mt-0.5 h-3 w-3 shrink-0 text-muted-foreground" /></button>)}</div> : <div className="mt-3 space-y-3">{claims.length === 0 ? <p className="py-8 text-center text-[10px] text-muted-foreground">{chinese ? "该维度还没有提交结构化结论。" : "No structured claims submitted yet."}</p> : claims.map((claim) => <article className="border-b border-border/70 pb-3" key={claim.id}><p className="text-[10px] leading-4">{claim.statement}</p><p className="mt-1 font-mono text-[8px] uppercase text-muted-foreground">{claim.confidence} · {claim.evidenceRefs.length} {chinese ? "条引用" : "citations"}</p>{claim.caveats.length > 0 ? <p className="mt-1 text-[9px] leading-4 text-[#8a6623]">{claim.caveats.join(" · ")}</p> : null}</article>)}</div>}
  </section>;
}
