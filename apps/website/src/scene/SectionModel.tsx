...
export function SectionModel({ src, label, variant = 'default' }: SectionModelProps) {
  ...
  useEffect(() => {
    ...
    root.rotation.set(variant === 'fold' ? -0.1 : 0.08, -0.25, variant === 'wide' ? -0.12 : 0.05)
    scene.add(root)
    let disposed = false
    new GLTFLoader().load(src, (gltf) => {
      ...
      const scale = (variant === 'wide' ? 3.8 : 3.1) / Math.max(size.x, size.y, size.z, 0.001)
    ...
    const render = () => {
      if (visible) {
        ...
        if (!reducedMotion) {
          ...
          root.rotation.y += (-0.25 + pointer.x * 0.22 + Math.sin(elapsed * 0.38) * 0.13 - root.rotation.y) * 0.035
          root.rotation.x += (0.08 - pointer.y * 0.12 - root.rotation.x) * 0.035
